"""
Fetches articles from industry-publication domains (TechCrunch, The Verge,
Ars Technica, etc.) via NewsAPI's domain filter, covering the spec's
"Industry Publications" source type. Inserts into raw_articles, reusing
the same table and downstream NLP pipeline as fetch_live_articles.py.
Run with: python scripts/fetch_industry_publications.py
"""
import sys
import os
import requests
...
INDUSTRY_DOMAINS = "techcrunch.com,theverge.com,arstechnica.com,engadget.com"


def fetch_from_publications(company: str, page_size: int = 3):
...
def insert_article(company: str, article: dict):
    ...
    db.run_query(
        ...
        """,
        {
            "source": article.get("source", {}).get("name", "Industry Publication"),
            "url": article.get("url", ""),
...
def fetch_all():
    ...
    for company in TRACKED_COMPANIES:
        articles = fetch_from_publications(company)
        inserted = 0
    ...
    print(f"\nDone. {total_inserted} new industry-publication articles inserted.")
