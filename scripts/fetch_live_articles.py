"""
Fetches live news articles from NewsAPI for tracked competitor companies
and inserts them into raw_articles (same table the manual seed script and
process_articles.py already use — no downstream changes needed).
Run with: python scripts/fetch_live_articles.py
"""
import sys
import os
import requests
...
def fetch_articles_for_company(company: str, page_size: int = 5):
    url = "https://newsapi.org/v2/everything"
    params = {
        "q": f"{company} smartphone",
        "language": "en",
        "sortBy": "publishedAt",
        "pageSize": page_size,
        "apiKey": NEWSAPI_KEY,
...
def fetch_all():
    ...
    for company in TRACKED_COMPANIES:
        ...
        for article in articles:
            ...
            inserted += 1
            total_inserted += 1
        print(f"{company}: fetched {len(articles)}, inserted {inserted} new so far")

    print(f"\nDone. {total_inserted} new live articles inserted into raw_articles.")
